...
const SidebarLink = (props) => {
    ...
    if (props.name === "Create Playlist") {
        ...
        icon = location === props.to
            ? ('sidebar-' + props.icon + '-active')
            : ('sidebar-' + props.icon);
        IconElement = () => { return (<Icon name={icon} color="#FFF" width={24} height={24} />) };
    }
    ...
    const to = (!appCtx.isLoggedIn && ((props.name === "Your Library") || (props.name === "Liked Songs"))) ? '#' : props.to;
        ...
        <Link to={to} className={`popover-wrapper ${styles.link}`}>
            ...
            {!appCtx.isLoggedIn && props.popoverContentTitle &&
                <div className="popover-content">
                    <div className="popover-content-title">{props.popoverContentTitle}</div>
